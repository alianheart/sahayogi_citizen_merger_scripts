...
order by foracid
 ...
 --9234 rows in third migration 

 IF OBJECT_ID('tempdb.dbo.#test5', 'U') IS NOT NULL
  DROP TABLE #test5;
select foracid,dmd_date,dmd_eff_date,dmd_flow_id,dmd_amt,late_fee_applied
  ,(ROW_NUMBER() OVER( PARTITION BY foracid,dmd_date,dmd_eff_date,dmd_flow_id,dmd_amt,late_fee_applied
ORDER BY foracid,dmd_amt)) as cont into #test5 from #test
