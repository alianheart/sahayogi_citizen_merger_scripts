...
Declare @ForAcid	varchar(20) ,	@BranchCode	varchar(20), @CyDesc	varchar(20), @MainCode	varchar(20), @AcType	varchar(20) , @LoanType varchar(20),
  ...
  ,@Next_Month_End date, @Next_Quarter_End date, @Next_HalfYear_End date, @Next_Year_End date,@expEMIDay varchar(3);
Begin
...
*/
					...
					begin
					--modified as date Dec 30 2018 email senthil/Durga.
						if @flow_start_date>=@MigDate							
						--case I=> within the expiry, ie EMI loan not expire					
						...
						--Case II =>  EMI LOan Expire 
						BEGIN
						
						--new logic 2019-01-27
						SELECT @expEMIDay= (SELECT day(RepayStartDate) from LoanMaster where MainCode=@MainCode)
						...
						insert into @LA_SCH(foracid	 ,flow_id	
												...
												case 
												 --when @RepayFreq='1' then 'D'
												 when @RepayFreq='5' then Cast((select '2019-04-'+@expEMIDay) as date)
												 when @RepayFreq='6' then Cast((select '2020-01-'+@expEMIDay) as date)
												 when @RepayFreq='7' then Cast((select '2019-07-'+@expEMIDay) as date)
												else Cast((select '2019-02-'+@expEMIDay) as date)
												end
												--,cast('31-DEC-2099' as date)-- @flow_start_date
												,@v_RepayFreq ,1
												,0.01		/* Need to confirm EMI Amt for schedule issue loan*/
												,null		--,@flow_start_date   -- For Expire Loan As dicussed Jun 5th
												,null --@MigDate			---@flow_start_date  
												,@v_RepayFreq
												, DatePart(Day,@flow_start_date),'P' );	
						END;					
				...
				begin		
					...
					BEGIN
						insert into @LA_SCH(foracid	 ,flow_id	
											...
											,@v_RepayFreq ,1
											,0.01			--@flow_amt
											,cast('31-DEC-2099' as date)		--,@flow_start_date   -- For Expire Loan As dicussed Jun 5th
						...
						insert into @LA_SCH(foracid	 ,flow_id	
											...
											,@v_RepayFreq ,0--1
											,0
											,NULL	--cast('30-DEC-2099' as date)		--,@flow_start_date		-- For Expire Loan As dicussed Jun 5th
											,NULL				--@flow_start_date		-- For Expire Loan As dicussed Jun 5th		
											, NULL --@v_RepayFreq		-- Interest freq dtls should not be entered for Principal / Int installments
											, DatePart(Day,@flow_start_date),'P' );
					end
...
*/			
			 ...
			 select  @ForAcid,  'PRDEM' flow_id, l.DueDate DueDate, @v_RepayFreq_Pri , 1 , DuePrincipal FlowAmt
								--, MIN(l.DueDate)	next_dmd_date  
								from PPIVSahayogiVBL.dbo.LoanRepaySched l
								 where  l.MainCode =@MainCode  
								 and DueDate>=@MigDate ;
								 --group by DueDate; ,DuePrincipalSEC:4
