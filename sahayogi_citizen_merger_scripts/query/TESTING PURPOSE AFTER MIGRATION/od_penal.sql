...
case when Flow_ID ='PIDEM' THEN SUM(isnull(IntOnPriOverDues,0))
...
Flow_ID,
sum(IntOverDues) as IntOverDues,
case when FLOWID ='OIDEM' THEN SUM(isnull(IntOnIntOverDues,0)) 
...
--Main Query
select MainCode, SOL_ID, CRNCY_CODE, VALUE_DATE, INT_DUE, PNLRATE_INTDUE, PNLINTAMT_INT, PRN_DUE,PNLINTAMT_PRN, TOTAL_PNL_AMT from (
SELECT 
t1.MainCode,
...
--where t1.MainCode = '00404300040325000003'
GROUP BY t1.BranchCode,t1.F_SolId,t1.ForAcid,t1.MainCode,t2.dmd_date,t3.IntDrRate)v
--order by t1.ForAcid
