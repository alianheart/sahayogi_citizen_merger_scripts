...
and MainCode = '00404300040325000003'
...
select  
	...
	,ct.CyDesc AS tran_crncy_code
	,m.F_SolId AS sol_id
	,CASE WHEN SUM(flow_amt_pidem)>0 THEN 'C' ELSE 'D' END AS part_tran_type
	,right(space(17)+cast(cast(abs(SUM(flow_amt_pidem)) as numeric(14,2))as varchar(17)),17) AS tran_amt
	,p.ReferenceNo +' '+o.NAME_OF_PRODUCT AS tran_particular
	,'' AS rpt_code
	,'' AS ref_num
	,'' AS instrmnt_type
